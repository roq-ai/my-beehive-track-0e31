...
import { getHealthChecks } from 'apiSdk/health-checks';
import { HealthCheckInterface } from 'interfaces/health-check';
import { getBees } from 'apiSdk/bees';
import { BeeInterface } from 'interfaces/bee';
import { getHarvests } from 'apiSdk/harvests';
import { HarvestInterface } from 'interfaces/harvest';

import { HarvestListPage as ShowCaseList } from 'pages/harvests';

function DashboardPage() {
  const { session } = useSession();

  const healthCheckStat = useSWR<PaginatedInterface<HealthCheckInterface>>(
    () => `/health-checks`,
    () => getHealthChecks({ limit: 1, offset: 0 }),
  );

  const beeStat = useSWR<PaginatedInterface<BeeInterface>>(
    () => `/bees`,
    () => getBees({ limit: 1, offset: 0 }),
  );

  const harvestStat = useSWR<PaginatedInterface<HarvestInterface>>(
    () => `/harvests`,
    () => getHarvests({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Health Checks',
      result: healthCheckStat?.data?.totalCount,
      isLoading: healthCheckStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Bees',
      result: beeStat?.data?.totalCount,
      isLoading: beeStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Harvests',
      result: harvestStat?.data?.totalCount,
      isLoading: harvestStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`harvests`} passHref legacyBehavior>
                  <Button
