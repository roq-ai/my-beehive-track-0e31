...
import { getHiveById, updateHiveById } from 'apiSdk/hives';
import { hiveValidationSchema } from 'validationSchema/hives';
import { HiveInterface } from 'interfaces/hive';
import { UserInterface } from 'interfaces/user';
import { getUsers } from 'apiSdk/users';

function HiveEditPage() {
  const router = useRouter();
  const id = router.query.id as string;

  const { data, error, isLoading, mutate } = useSWR<HiveInterface>(
    () => (id ? `/hives/${id}` : null),
    () => getHiveById(id),
  );
  ...
  const handleSubmit = async (values: HiveInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateHiveById(id, values);
      mutate(updated);
      resetForm();
      router.push('/hives');
    } catch (error: any) {
